...
-   Te recomiendo resolver estos ejercicios en tu computador y luego compararlas con las soluciones que postearé a su debido tiempo
:::
...
**2.    Lleva a cabo un ANOVA de dos vías no aditivo con los datos `Ethanol` de `{Stat2Data}`. Estos datos corresponden a un experimento que fue diseñado con el fin de comparar el efecto ihnibitorio del oxígeno sobre el metabolismo de dos tipos distintos de azúcar (glucosa y galactosa) por parte de bacterias *Streptococuss* usando cuantro niveles distintos de oxígeno.**
...
library(patchwork)
data("Ethanol")
Ethanol$O2Conc <- as.factor(Ethanol$O2Conc)
p1 <- Ethanol |>
  ggplot(aes(x = Sugar, y = Ethanol)) +
  geom_boxplot()
p2 <- Ethanol |>
  ggplot(aes(x = O2Conc, y = Ethanol)) +
  geom_boxplot()
p1+p2
```

A un primer vistazo, que existen diferencias en la producción de etanol cuando estas bacterias están en un medio con galactosa o glucosa así como también en cuanto a la disponibilidad de oxígeno.

**2.    Llevo a cabo el ANOVA de una vía y chequeo los supuestos**

```{r echo=T, eval=T, error=T, fig.align = 'center', message=F, warning=F}
library(car)
etho <- lm(Ethanol ~ Sugar * O2Conc, data = Ethanol)
# normalidad de los residuales
residuos <- etho$residuals
shapiro.test(residuos)
# homogeneidad de la varianza
leveneTest(etho)
```
...
En este caso, solo uno de los efectos principales es significativo: `Sugar`

**5.    Llevo a cabo gráficos de interacción**

```{r echo=T, eval=T, error=T, fig.align = 'center', message=F, warning=F}
library(emmeans)
p3 <- emmip(etho, Sugar ~ O2Conc)
p4 <- emmip(etho, O2Conc ~ Sugar)
p3 + p4
```

Reafirmamos que la interacción no es significativa ya que las líneas de interacción si se llegasen a encontrar, esto sucedería a predicciones de etanol negativas.

**4.    Llevo a cabo comparaciones múltiples**

En este caso ya no resulta necesario, como el efecto principal que estadísticamente significativo y este posee únicamente dos niveles, es fácil distinguir a partir del primer gráfico que hicimos en nuestro breve análisis exploratorio que las bacterias en un medio de cultivo de glucosa producen más etanol con significancia estadística que con respecto a las que crecen en un medio con galactosa.
